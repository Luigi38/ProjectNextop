...
int convert_to_number(const string& alphabet) {
	int addit = pow(26, alphabet.size() - 1);
	int cases = 0;

	for (int i = 0; i < alphabet.size(); i++) {
		if (i != alphabet.size() - 1) {
			cases += (int)pow(26, alphabet.size() - i - 1) * (alphabet[i] - 'A');
		}
		else {
			cases += alphabet[i] - 'A' + 1;
		}
	}
...
string convert_to_alphabet(const string& number) {
	...
	int divided = stoi(number) / 26;
	int moded = stoi(number) % 26;

	while (divided >= 26) {
		alphabet += convert_moded(divided % 26);
		divided /= 26;
	}
	alphabet += convert_moded(divided);
	alphabet += convert_moded(moded);
