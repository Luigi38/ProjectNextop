...
vector<NutCase2> higher_than_minimum(const vector<Nut>& nuts, const vector<vector<int>>& indicies, const Nut& minimum) { //this function is normal
	vector<NutCase2> v;
	vector<NutCase2> v2;
...
NutOutput diet(const vector<Nut>& nuts, const int N, const Nut& minimum) { //this function is only problem
	...
	for (int i = 0; i < 15; i++) { //Why "i < 15" ???
		NutCase nc = { i, nuts, vector<bool>(), 0, vector<vector<int>>() };
		dfs(0, true, nc);
		...
		nc = { i, nuts, vector<bool>(), 0, vector<vector<int>>() };
		dfs(0, false, nc);
...
int main() { //this function is normal
	...
	operator + : Normal
	dfs() : Normal
	higher_than_minimum() : Normal
	compare_by_cost() : Normal
	diet() : Problem
	main() : Normal
	*/
